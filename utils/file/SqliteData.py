...
async def set_guild_conf(guild_id:str,admin_user:list[str],channel_id:str,role_id="e"):
    """配置服务器键值，如果已有，执行更新
    ...
    async with DBSqlLock:
        with sqlite3.connect(DB_NAME) as db:
            query = db.cursor()
            select_ret = query.execute(
                SqliteSql.Select.SELECT_GUILD_CONF, (guild_id, ))
            select_ret_all = select_ret.fetchall()
            if not select_ret_all: # 没有找到
                query.execute(SqliteSql.Insert.INSERT_GUILD_CONF,(guild_id,json.dumps(admin_user),channel_id,role_id))
            else: # 找到了
                is_insert = False
                admin_user_list = json.loads(
                    select_ret_all[0][1])  # 原本的管理员用户列表
                ...
                # 更新
                query.execute(SqliteSql.Update.UPDATE_GUILD_CONF,(admin_user_list,channel_id,role_id,guild_id))
